...
这个版本使用本机官方 Codex CLI，复用你已经完成的 ChatGPT 登录状态，不需要在网页后台填写 API Key。后台可读取账号模型、切换思考强度并按间隔生成；每个任务使用下方固定提示词，生成一个包含 SVG 鹈鹕动画的 HTML，结果会保存到画廊。

每次生成都使用 Codex CLI 的独立 `--ephemeral` 会话，并从临时工作目录开始；任务完成或失败后临时目录自动删除，不复用上一轮上下文。固定提示词由公开页面、实际请求和每轮记录共用，调用层不会增加额外提示词前缀。后台会读取本机登录缓存中的账号标识生成脱敏指纹，刷新后台时可识别账号切换；如果一轮执行中途切换账号，该轮会停止并要求重新开始。
...
- **单图生成**：每次从独立新会话开始，使用固定提示词生成一个 SVG 鹈鹕骑行 HTML 动画；仅请求成功或失败会影响状态，画面由用户在画廊人工判断。
...
- **SVG 下载**：画廊卡片、详情及后台记录均可下载已有 SVG。导出会保留可安全内嵌的 HTML 样式和原始内联动画脚本，并按 XML 格式转义；预览用的 SVG 仍不含脚本，原始输出不改写。脚本动画需用 Chrome / Edge 等浏览器单独打开 SVG；作为普通图片或 `<img>` 使用时脚本不执行。外部脚本不打包，依赖 HTML 页面结构或外部库的作品可下载完整 HTML 查看。
...
- **后台账号标记**：每个新 Codex 请求保存开始时的账号指纹，切换账号不会改写已有记录。旧记录显示“账号未记录”，API 节点显示“API 节点”；账号标记仅向已登录管理员返回。画廊和后台均支持按账号筛选展示与跨页全选删除；切换筛选会清空旧选择，删除接口再次核对账号范围。后台还支持模型、状态、循环组、时间与关键词筛选，每页 20 条。
...
结果状态只反映模型请求是否成功；生成内容的画面效果请在画廊人工判断。默认生成间隔为 30 分钟，自动检测初始关闭。
...
每次请求使用固定提示词，并通过 Codex CLI `--ephemeral` 启动独立会话，生成一个含 SVG 的 HTML 动画。状态只表示请求是否成功，不判断 SVG 格式、动画效果或画面质量；详情页保留安全预览、原始输出和提示词，画面请在画廊人工判断。请求成功不证明模型身份，也不代表模型的整体能力。
